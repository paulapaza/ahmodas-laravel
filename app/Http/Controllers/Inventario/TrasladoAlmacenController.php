...
class TrasladoAlmacenController extends Controller
{
    ...
    public function getDataGestionApi()
    {
        ...
        $tiendas = DB::table('tiendas')
            ...
            ->select('id', 'nombre')
            ->get();
        // Productos con su stock en el Almacén Principal (ID: 1)
        $productos = DB::table('productos')
            ->leftJoin('producto_tienda', function($join) {
    ...
    public function store(TrasladoAlmacenStoreRequest $request)
    {
        try {
            ...
            // Consultamos stock actual de todos los productos involucrados en el Almacén Principal (tienda_id = 1)
    ...
    public function actualizarStock(Request $request)
    {
        try {
            ...
            if ($cantidadAgregar <= 0) throw new \Exception("La cantidad a agregar debe ser mayor a 0.");

            DB::beginTransaction();

            $traslado = DB::table('almacen_traslados')->where('id', $trasladoId)->lockForUpdate()->first();
            if (!$traslado) throw new \Exception("Traslado no encontrado.");

            // 1. Validar Stock en Almacén Principal (tienda_id = 1)
    ...
    public function eliminarTraslado(Request $request)
    {
        try {
            ...
            // 3. Eliminar (SoftDelete)
            DB::table('almacen_traslados')->where('id', $trasladoId)->update(['deleted_at' => now()]);

            // 4. Historial de eliminación (para trazabilidad del retorno al almacén)
            DB::table('almacen_traslados_historial')->insert([
                'almacen_traslado_id' => $trasladoId,
                'stock_disponible' => 0,
                'stock_vendido' => $traslado->stock_vendido,
                'stock_almacen' => DB::table('producto_tienda')->where('producto_id', $traslado->producto_id)->where('tienda_id', 1)->value('stock'),
                'created_at' => now(),
                'updated_at' => now()
            ]);
    ...
    public function actualizarVenta(Request $request)
    {
        try {
            ...
            if ($nuevaVenta < 0) throw new \Exception("La cantidad no puede ser negativa.");
    ...
    public function actualizarDevolucion(Request $request)
    {
        try {
            ...
            if ($cantidadRegresar <= 0) throw new \Exception("La cantidad a regresar debe ser mayor a 0.");
            ...
            // 1. Devolver al Almacén Principal (tienda_id = 1)
            DB::table('producto_tienda')
                ->where('producto_id', $traslado->producto_id)
                ->where('tienda_id', 1)
