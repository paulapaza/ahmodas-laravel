<x-admin-layout>
    ...
    <x-slot name="pagetitle">Historial de Traslados desde Tiendas</x-slot>
    ...
    <div id="historial-traslados-tiendas-app" v-cloak>
            ...
            <div class="row mb-4">
                <div class="col-12 d-flex justify-content-between align-items-center">
                    <h4 class="mb-0 font-weight-bold text-dark">
                        <i class="fas fa-history mr-2 text-success"></i> HISTORIAL: TIENDAS ➔ ALMACÉN
                    </h4>
                    <a href="{{ route('inventario.traslados_tiendas.index') }}" class="btn btn-success shadow-sm px-4">
                ...
                <div class="card-body p-0">
                    <div class="filter-bar">
                        <div class="row align-items-end">
                            <div class="col-md-3">
                                <label class="small font-weight-bold text-muted uppercase">Tienda Origen:</label>
                        ...
                        <table class="table table-hover table-striped table-sm mb-0 table-sm-text">
                            <thead class="bg-light text-muted uppercase">
                                <tr>
                                    ...
                                    <th class="py-3 border-top-0">Tienda Origen</th>
                                    <th class="py-3 border-top-0">Producto</th>
                                    <th class="text-center py-3 border-top-0">Cantidad</th>
                                    <th class="text-center py-3 border-top-0">Stock Tienda (Antes/Desp)</th>
                                    <th class="text-center py-3 border-top-0">Stock Almacén (Antes/Desp)</th>
                            ...
                            <tbody>
                                ...
                                <tr v-else v-for="t in traslados" :key="t.traslado_id">
                                    ...
                                    <td class="text-center align-middle" style="font-size: 0.95rem;">
                                        <span class="text-muted small">@{{ t.tienda_stock_anterior }} ➔</span>
                                        <span class="font-weight-bold text-danger">@{{ t.tienda_stock_posterior }}</span>
                                    </td>
                                    <td class="text-center align-middle" style="font-size: 0.95rem;">
                                        <span class="text-muted small">@{{ t.almacen_stock_anterior }} ➔</span>
                                        <span class="font-weight-bold text-primary">@{{ t.almacen_stock_posterior }}</span>
                                    </td>
    ...
    <script>
        new Vue({
            el: '#historial-traslados-tiendas-app',
            data() {
                return {
                    tiendas: [],
                    traslados: [],
            ...
            methods: {
                fetchInitialData() {
                    axios.get('/api/inventario/traslados-tiendas/datos-gestion')
                        .then(res => { this.tiendas = res.data.tiendas; });
                },
                async fetchHistorial() {
                    this.cargando = true;
                    try {
                        const response = await axios.get('/api/inventario/traslados-tiendas/historial-global', {
                            params: this.filtros
                        });
                ...
                resetFiltros() {
                    this.filtros = { tienda_id: '', fecha: '', search: '' };
                    this.fetchHistorial();
                }
            }
        });
