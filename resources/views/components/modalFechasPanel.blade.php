    ...
    <script>
        ...
        $('#modal-filter-fechas').on('click', '.filtrar-fechas', function() {
            ...
            if (!fechaInicio || !/^\d{4}-\d{2}-\d{2}$/.test(fechaInicio)) {
                return mostrarError('La fecha de inicio es obligatoria y debe tener formato válido (YYYY-MM-DD)',
                    '#fecha_inicio');
            }

            try {
                const hoy = Temporal.Now.plainDateTimeISO('America/Lima').toPlainDate();
                const fechaInicioDate = Temporal.PlainDate.from(fechaInicio);

                // Validar que no sea anterior a hoy
                if (Temporal.PlainDate.compare(fechaInicioDate, hoy) > 0) {
                    return mostrarError('La fecha de inicio no puede ser posterior a hoy', '#fecha_inicio');
                }

                // Validar fecha fin si existe
                if (fechaFin) {
                    if (!/^\d{4}-\d{2}-\d{2}$/.test(fechaFin)) {
                        return mostrarError('La fecha de fin debe tener formato válido (YYYY-MM-DD)', '#fecha_fin');
                    }

                    const fechaFinDate = Temporal.PlainDate.from(fechaFin);
                    if (Temporal.PlainDate.compare(fechaFinDate, fechaInicioDate) < 0) {
                        return mostrarError('La fecha de fin no puede ser anterior a la fecha de inicio',
                            '#fecha_fin');
                    }
                }
                ...
                mostrarError('Error en las fechas ingresadas', '#fecha_inicio');
            }
        });
